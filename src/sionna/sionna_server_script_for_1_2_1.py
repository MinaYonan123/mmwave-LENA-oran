...
def manage_location_message(message, sionna_structure):
    ...
    try:
        ...
        # Accept both formats:
        #   obj,x,y,z,angle
        #   obj,x,y,z,angle,vx,vy,vz
        if len(parts) == 5:
            parts.extend(["0.0", "0.0", "0.0"])
        elif len(parts) < 8:
            if sionna_structure["verbose"]:
                print(f"EXCEPTION - Location parsing failed: expected 5 or 8 fields, got {len(parts)}")
            return None
    ...
    for tx_idx, src_idx in enumerate(source_indices):
        ...
        for rx_idx, tgt_idx in enumerate(target_indices):
            ...
            try:
                ...
                is_los = False
                for p in np.where(valid_mask)[0]:
                    if np.all(interaction_types[:, p] == 0):
                        is_los = True
                        break
