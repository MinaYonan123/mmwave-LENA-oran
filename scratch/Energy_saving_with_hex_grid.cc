    ...
    for (NodeList::Iterator it = NodeList::Begin(); it != NodeList::End(); ++it) {
        ...
        for (int j = 0; j < nDevs; j++) {
            ...
            if (enbdev) {
                ...
                std::ofstream outFileMain(filename1, std::ios_base::out | std::ios_base::app);
                outFileMain << timestamp << "," << enbdev->GetCellId() << "," << pos.x << "," << pos.y
                            << "," << m_startTime << "," << "0" << "," << "30" << std::endl;
                outFileMain.close();

                // --- SUBFOLDER output ---
                std::ofstream outFileFolder((folderName.str() + "/" + filename1),
                                            std::ios_base::out | std::ios_base::app);
                outFileFolder << timestamp << "," << enbdev->GetCellId() << "," << pos.x << "," << pos.y
                              << "," << m_startTime << "," << "0" << "," << "30" << std::endl;
                outFileFolder.close();
                ...
                // --- MAIN directory output ---
                std::ofstream outFileMain(filename2, std::ios_base::out | std::ios_base::app);
                outFileMain << timestamp << "," << cell_id << "," << pos.x << "," << pos.y << ","
                ...
                // --- SUBFOLDER output ---
                std::ofstream outFileFolder(folderName.str() + "/" + filename2,
                                            std::ios_base::out | std::ios_base::app);
                outFileFolder << timestamp << "," << cell_id << "," << pos.x << "," << pos.y << ","
                              << m_startTime << "," << esON_list[cell_id] << ","
                              << current_energy_consumption[cell_id] << "," << max_energy_consumption
                              << "," << sum_curr_total_energy_consumption << std::endl;
                outFileFolder.close();
...
void
ClearFile(const std::string &Filename, uint64_t m_startTime) {
    ...
    if (Filename == "ue_position.txt") {
        outFile << "timestamp,id,x,y,type,cell,simid" << std::endl;
    } else { // enbs / gnbs files
        outFile << "timestamp,id,x,y,simid,ESstate,currEC,maxEC,totalcurrEC" << std::endl;
        outFile << m_startTime << "," << "0" << "," << maxXAxis << "," << maxYAxis << std::endl;
    }
...
void
PrintPosition(Ptr<Node> node, int iterator, const std::string &Filename, uint64_t m_startTime) {
    uint64_t timestamp = m_startTime + (uint64_t) Simulator::Now().GetMilliSeconds();
    ...
    std::stringstream folderFilePath;
    folderFilePath << folderName.str() << "/" << Filename;
    ...
    for (int j = 0; j < nDevs; j++) {
        ...
        if (mcuedev) {
            ...
            NS_LOG_UNCOND("Position of UE with IMSI " << imsi << " is " << position
            ...
            // Append to main file
            {
                ...
                outFile << timestamp << "," << imsi << "," << position.x << "," << position.y
                        << ",mc," << serving_cell << "," << m_startTime << std::endl;
            }

            // Append to folder
            {
                std::ofstream outFile(folderFilePath.str(), std::ios_base::out | std::ios_base::app);
                outFile << timestamp << "," << imsi << "," << position.x << "," << position.y
                        << ",mc," << serving_cell << "," << m_startTime << std::endl;
            }
        }
...
EnergyConsumptionUpdate(int nodeIndex, std::string filename, double totaloldEnergyConsumption,
    ...
    std::ofstream outFile;
    outFile.open(filename, std::ios_base::out | std::ios_base::app);  // ✅ filename already includes folder
...
void AllocateHexagonalSites(Ptr<ListPositionAllocator> positionAlloc,
                            uint32_t numRings, double radius, double zHeight,
                            double centerX, double centerY)
{
    ...
    // Add center site (0,0) shifted to precalculated center
    positionAlloc->Add(Vector(centerX, centerY, zHeight));

    // Generate rings
    for (uint32_t ring = 1; ring <= numRings; ++ring) {
        int q = dirQ[4] * (int)ring;
        int r = dirR[4] * (int)ring;

        for (int side = 0; side < 6; ++side) {
            for (int step = 0; step < (int)ring; ++step) {
                // Convert axial coords (q,r) to Cartesian (x,y)
                double x = radius * std::sqrt(3.0) * (q + r / 2.0);
                double y = radius * 1.5 * r;

                // Apply precalculated center
                positionAlloc->Add(Vector(x + centerX, y + centerY, zHeight));

                // Move one step in this side
                q += dirQ[side];
                r += dirR[side];
            }
        }
    ...
    std::cout << "Hexagonal grid allocated around center: ("
              << centerX << ", " << centerY << ", " << zHeight << ")" << std::endl;
}


void AllocateUesAroundCells(Ptr<ListPositionAllocator> uePositionAlloc,
                            NodeContainer gnbNodes,
                            uint32_t numUesPerCell,
                            double ueDiscRadius)
{
    ...
    for (uint32_t i = 0; i < gnbNodes.GetN(); ++i)
    {
        ...
        Vector gnbPos = gnb->GetObject<MobilityModel>()->GetPosition(); // gNB position

        NS_LOG_UNCOND("gNB[" << i << "] position: x=" << gnbPos.x
                             << ", y=" << gnbPos.y
                             << ", z=" << gnbPos.z);
        //NS_LOG_UNCOND("numUesPerCell" << numUesPerCell);
        for (uint32_t u = 0; u < numUesPerCell; ++u)
        {
            double theta = angle->GetValue(0, 2 * M_PI);
            double r     = std::sqrt(rho->GetValue(0, 1.0)) * ueDiscRadius;
            ...
            NS_LOG_UNCOND("UE[" << u << "] around gNB[" << i << "] allocated at: x="
                                << x << ", y=" << y << ", z=" << z);
        }
    }
}
...
static ns3::GlobalValue ue_per_cell("ue_per_cell", "Number of User Equipments per cell",
                             ns3::UintegerValue(2),
                             ns3::MakeUintegerChecker<uint32_t>());
...
static ns3::GlobalValue num_of_hex_rings("numRings", "num_of_hex_rings",
                                         ns3::UintegerValue(1),
                                         ns3::MakeUintegerChecker<uint32_t>());

static ns3::GlobalValue radius_of_rings("radius_rings", "Interside Distance Value",
...
main(int argc, char *argv[]) {
    ...
    // uint32_t numRings = 1;
    GlobalValue::GetValueByName ("numRings", uintegerValue);
    uint32_t numRings = uintegerValue.Get();

    //  double radius_rings = 1000.0;
    GlobalValue::GetValueByName("radius_rings", doubleValue);
    double radius_rings = doubleValue.Get();

    GlobalValue::GetValueByName ("ue_per_cell", uintegerValue);
    uint32_t ues = uintegerValue.Get();

    uint8_t nMmWaveEnbNodes = 1 + 3 * numRings * (numRings + 1);
    uint8_t nUeNodes = ues * nMmWaveEnbNodes;

    double centerX = sqrt(3.0) * numRings * radius_rings + radius_rings;
    double centerY = 1.5 * numRings * radius_rings + radius_rings;
    ...
    Config::SetDefault ("ns3::LteEnbPhy::TxPower", DoubleValue (100)); //to cover whole scenario, module do not support HOs between anchors so no need to place more than one
    ...
    // Position
    Vector centerPosition = Vector(centerX, centerY, 3);
...
*/
    ...
    AllocateHexagonalSites(positionAlloc, numRings, radius_rings, 30.0, centerX, centerY);
    ...
    // Allocate UEs
    uint32_t numUesPerCell = ues;
    double ueDiscRadius = radius_rings * 0.3; // UE cluster radius around gNB

    Ptr<ListPositionAllocator> uePositions = CreateObject<ListPositionAllocator>();
    AllocateUesAroundCells(uePositions, mmWaveEnbNodes, numUesPerCell, ueDiscRadius);
...
//}
    ...
    // Create output energy trace files inside trace_<m_startTime> folder
    std::stringstream energyFolder;
    energyFolder << "trace_" << t_startTime_simid;   // ✅ same folder as before
    if (!std::filesystem::exists(energyFolder.str())) {
        std::filesystem::create_directory(energyFolder.str());
    }

    std::vector<std::ofstream> outFiles;
    for (int x = 0; x < nMmWaveEnbNodes; ++x) {
        std::ostringstream energyFileName;
        energyFileName << energyFolder.str() << "/energyfilecell" << x + 2 << ".csv";  // ✅ include folder path
    ...
    for (int x = 0; x < nMmWaveEnbNodes; ++x) {
        ...
        filename << energyFolder.str() << "/energyfilecell" << x + 2 << ".csv";  // ✅ include folder path
