...
main(int argc, char* argv[])
{
    ...
    Time simTime = MilliSeconds(8000);
    uint16_t gnbUeDistance = 100; // meters
    ...
    cmd.AddValue("sionna-verbose",
                 "Enable verbose logging to compare values from Sionna and ns-3 channel model",
                 verb);
    ...
    if (sionna)
    {
        ...
        sionnaHelper.SetLocalMachine(local_machine); // Set True if Sionna is running locally
   ...
   // LogComponentEnable("SionnaConnectionHandler", LOG_LEVEL_DEBUG);
    ...
    if (enableInterfNode)
    {
        positionAlloc->Add(Vector(0.0, interfDistance, 5.0));
    ...
    // Install WaypointMobilityModel on UE
    MobilityHelper ueMobility;
    ueMobility.SetMobilityModel("ns3::WaypointMobilityModel");
    ueMobility.Install(ueContainer);

    // Get the installed mobility model
    Ptr<WaypointMobilityModel> ueWp =
        ueContainer.Get(0)->GetObject<WaypointMobilityModel>();

    // Explicit initial position at t = 0.0 s
    ueWp->AddWaypoint(Waypoint(Seconds(0.0), Vector(150.0, 0.0, 1.5)));

    // Movement trajectory
    ueWp->AddWaypoint(Waypoint(Seconds(1.0), Vector(150.0, -100.0, 1.5)));
    ueWp->AddWaypoint(Waypoint(Seconds(2.0), Vector(0.0, -100.0, 1.5)));
    ueWp->AddWaypoint(Waypoint(Seconds(3.0), Vector(-150.0, -100.0, 1.5)));
    ueWp->AddWaypoint(Waypoint(Seconds(4.0), Vector(-150.0, 0.0, 1.5)));
    ...
    for (uint32_t numCell = 0; numCell < gnbContainer.GetN(); ++numCell)
    {
        ...
        // double downtiltDeg = 3;     // Column 3 is downtilt in degrees
