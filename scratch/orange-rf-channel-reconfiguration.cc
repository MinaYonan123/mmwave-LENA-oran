...
PrintGnuplottableEnbListToFile()
{
    ...
    for (NodeList::Iterator it = NodeList::Begin(); it != NodeList::End(); ++it)
    {
        ...
        for (int j = 0; j < nDevs; j++)
        {
            ...
            outFile1 << timestamp << "," << nrdev->GetCellId() - cell_it << "," << pos.x << ","
                     << pos.y << "," << t_startTime_simid << "," << 0 << "," << 0 << "," << 0 << ","
                     << 0 << std::endl;
...
PrintPosition(Ptr<Node> node, std::string Filename)
{
    ...
    for (int j = 0; j < nDevs; j++)
    {
        ...
        if (nruedev)
        {
            ...
            NS_LOG_UNCOND(std::noshowpos
                          << std::fixed << std::setprecision(2) << "Position of UE with IMSI "
                          << std::dec << static_cast<uint32_t>(imsi) << " (UE_ID in GUI: "
                          << static_cast<uint32_t>(gui_ue_id) << ") is " << position.x << ":"
                          << position.y << ":" << position.z << ", Speed: " << speedKmh << " km/h"
                          << " at time " << Simulator::Now().GetSeconds()
                          << ", UE connected to Cell: " << std::dec
                          << static_cast<uint32_t>(serving_cell));
            ...
            outFile << timestamp << "," << static_cast<uint32_t>(gui_ue_id) << "," << position.x
                    << "," << position.y << ",nr,"
                    << static_cast<uint32_t>((serving_cell - 1) / 2 + 1) << "," << t_startTime_simid
                    << std::endl;
...
main(int argc, char* argv[])
{
    ...
    // GUI related flags
    cmd.AddValue("enableE2FileLogging",
                 "Do not use E2 interface, instead, produce file trace for KPIs",
                 enableE2FileLogging);
    cmd.AddValue("hoSinrDifference",
                 "hoSinrDifference",
                 hoSinrDifference); // not implemented in Lena
    ...
    cmd.AddValue("IntersideDistanceUEs",
                 "Interside Distance Value",
                 isd_ue); // not supported due to GRID allocation
    cmd.AddValue("IntersideDistanceCells",
                 "Interside Distance Value",
    ...
    // This guarantee that each of the rest BSs is placed at the same distance from the two
    // co-located in the center
    ...
    NodeContainer ueContainer;
    ueContainer.Create(4);
    Ptr<ListPositionAllocator> uePositionAlloc =
        CreateObject<ListPositionAllocator>();

    // Sector 1
    uePositionAlloc->Add(Vector(1300.0, 2000.0, 1.5));
    uePositionAlloc->Add(Vector(1300.0, 1800.0, 1.5));

    // Sector 2
    uePositionAlloc->Add(Vector(2700.0, 2000.0, 1.5));
    uePositionAlloc->Add(Vector(2700.0, 2200.0, 1.5));
    ...
    uemobility.SetMobilityModel("ns3::RandomWalk2dOutdoorMobilityModel", "Speed",
                                ...
                                RectangleValue(Rectangle(1000, 3000, 1000, 3000)));
    ...
    for (uint32_t numCell = 0; numCell < gnbNetDev.GetN(); ++numCell)
    {
        ...
        nrHelper->GetGnbPhy(gnbNetDev.Get(numCell), 0)->SetAttribute("TxPower", DoubleValue(35));
    }
